...
class Refine_block2_1(nn.Module):
    ...
    def forward(self, *input):
        ...
        if self.factor > 1:
            padding, out_padding = utils.get_padding(x1.size(), x2.size(), self.factor)
...
class Refine_block3_1(nn.Module):
    def __init__(self, in_channel, out_channel, require_grad=False):
        ...
        self.deconv_weight1 = nn.Parameter(utils.bilinear_upsample_weights(2, out_channel), requires_grad=require_grad)
        self.deconv_weight2 = nn.Parameter(utils.bilinear_upsample_weights(4, out_channel), requires_grad=require_grad)
...
class decode(nn.Module):
    def __init__(self, cfgs):
        ...
        self.cfgs = cfgs
        if 'vgg' in cfgs['backbone']:
            ch = {'L1_1': [(64, 256), 64, 4],
                'L1_2': [(128, 512), 128, 4],
                'L1_3': [(256, 512), 256, 4]}
        else:
            if 'BSDS' in cfgs['dataset']:
                ch = {'L1_1': [(64, 512), 64, 4],
                    'L1_2': [(256, 1024), 128, 2],
                    'L1_3': [(512, 2048), 256, 1]}
            elif 'NYUD' in cfgs['dataset']:
                ch = {'L1_1': [(64, 512), 64, 4],
                    'L1_2': [(256, 1024), 128, 4],
                    'L1_3': [(512, 2048), 256, 2]}
...
class DRNet(nn.Module):
    def __init__(self, cfgs):
        ...
        self.encode = backbone.backbone(cfgs['backbone'], cfgs)
        self.decode = decode(cfgs)
    def forward(self, x):
        end_points = self.encode(x)
        end_points = [end_points['feat1'], end_points['feat2'], end_points['feat3'], 
                    end_points['feat4'], end_points['feat5']]
...
class Loss(nn.Module):
    ...
    def forward(self, pred, labels):
        if self.cfgs['loss'] == 'CE':
            total_loss = cross_entropy_per_image(pred, labels)
        elif self.cfgs['loss'] == 'Dice':
            total_loss = self.cfgs['c'] * dice_loss_per_image(pred, labels)
        elif self.cfgs['loss'] == 'DF':
            total_loss = self.cfgs['a'] * cross_entropy_per_image(pred, labels) + \
                        self.cfgs['b'] * self.cfgs['c'] * dice_loss_per_image(pred, labels)
        elif self.cfgs['loss'] == 'AF':
            total_loss = self.weight1.pow(-2) * cross_entropy_per_image(pred, labels) + \
                     self.weight2.pow(-2) * self.cfgs['c'] * dice_loss_per_image(pred, labels) + \
        ...
        # total_loss = cross_entropy_per_image(pred, labels) + \
        #              0.1 * dice_loss_per_image(pred, labels)
        # total_loss = self.weight1.pow(-2) * cross_entropy_per_image(pred, labels) + \
